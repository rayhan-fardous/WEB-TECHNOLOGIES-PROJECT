...
function admin_pendingstudents()
{
    $connection=new db();
    $connobj=$connection->OpenCon();
    $connection->admin_showpendingstudents($connobj,"pending");
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
    if(isset($_REQUEST["acceptid"]) && isset($_REQUEST["accept"]))
    {
        ...
        else if(is_numeric($_REQUEST["acceptid"]))
        {
            $connection=new db();
            $connobj=$connection->OpenCon();
            $result=$connection->admin_getuserbyid($connobj,"students",$_REQUEST["acceptid"]);
    ...
    if(isset($_REQUEST["rejectid"]) && isset($_REQUEST["reject"]))
    {
        ...
        else if(is_numeric($_REQUEST["rejectid"]))
        {
            $connection=new db();
            $connobj=$connection->OpenCon();
            $result=$connection->admin_getuserbyid($connobj,"students",$_REQUEST["rejectid"]);
            if($result->num_rows>0)
            {
                $row=$result->fetch_assoc();
                $id=$row["id"];
                ...
                {
                    if($connection->admin_updateUserStatus($connobj,"users",$id,"rejected") && $connection->admin_updateUserStatus($connobj,"students",$id,"rejected"))
                    {
