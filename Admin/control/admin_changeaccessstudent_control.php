...
$active=$connection->admin_showallstudents($connobj,"active");
$blocked=$connection->admin_showallstudents($connobj,"blocked");
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
    ...
    if(isset($_REQUEST["activeid"]) && isset($_REQUEST["active"]))
    {
        ...
        else if(is_numeric($_REQUEST["activeid"]))
        {
            $connection=new db();
            $connobj=$connection->OpenCon();
            $result=$connection->admin_getuserbyid($connobj,"students",$_REQUEST["activeid"]);
            if($result->num_rows>0)
            {
                ...
                {
                    if($connection->admin_updateUserStatus($connobj,"users",$id,"active") && $connection->admin_updateUserStatus($connobj,"students",$id,"active"))
                    {
    ...
    if(isset($_REQUEST["blockid"]) && isset($_REQUEST["block"]))
    {
        ...
        else if(is_numeric($_REQUEST["blockid"]))
        {
            $connection=new db();
            $connobj=$connection->OpenCon();
            $result=$connection->admin_getuserbyid($connobj,"students",$_REQUEST["blockid"]);
            if($result->num_rows>0)
            {
                $row=$result->fetch_assoc();
                $id=$row["id"];
                ...
                {
                    if($connection->admin_updateUserStatus($connobj,"users",$id,"blocked") && $connection->admin_updateUserStatus($connobj,"students",$id,"blocked"))
                    {
