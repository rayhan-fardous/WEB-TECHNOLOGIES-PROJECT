...
$active=$connection->admin_showallteachers($connobj,"active");
$blocked=$connection->admin_showallteachers($connobj,"blocked");
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
    ...
    if(isset($_REQUEST["activeid"]) && isset($_REQUEST["active"]))
    {
        ...
        else if(is_numeric($_REQUEST["activeid"]))
        {
            $connection=new db();
            $connobj=$connection->OpenCon();
            $result=$connection->admin_getuserbyid($connobj,"teachers",$_REQUEST["activeid"]);
    ...
    if(isset($_REQUEST["blockid"]) && isset($_REQUEST["block"]))
    {
        ...
        else if(is_numeric($_REQUEST["blockid"]))
        {
            $connection=new db();
            $connobj=$connection->OpenCon();
            $result=$connection->admin_getuserbyid($connobj,"teachers",$_REQUEST["blockid"]);
            if($result->num_rows>0)
            {
                $row=$result->fetch_assoc();
                $id=$row["id"];
                ...
                {
                    if($connection->admin_updateUserStatus($connobj,"users",$id,"blocked") && $connection->admin_updateUserStatus($connobj,"teachers",$id,"blocked"))
                    {
